var newComponent = (function(){
   ...
   attachmentStore.on("beforeload",function(a,b){
    	pId = -1;
    	softIndexStore.removeAll();
    });
    ...
    var addPhonetypeCombo = new Ext.form.ComboBox({ 
		...
		valueField : 'phonetypeId',// 域的值,对应于store里的fields 
		displayField : 'phonetypeName',// 显示的域,对应于store里的fields 
		hideOnSelect:false,
		triggerAction:'all',
		editable : false,
		allowBlank:false,
		hiddenName:"phonetypeId",
		mode:'local'
	});
    ...
    var softIndexGrid = new Ext.grid.GridPanel({
        ...
        tbar:[{
             ...
             handler : function(){
               ...
               Ext.MessageBox.confirm("提示","您确定要删除该项吗?",function(bool){
               		var indexId = softIndexStore.getAt(cId).data.indexId;
               		if(bool == 'yes'){
               			Ext.Ajax.request({
    ...
    var panel = new Ext.Panel({
     ...
     panel.on('beforeclose',function(p,o){
    	softindexAddWindow.close();
    	softindexUpdateWindow.close();
    	attachmentUpdateWindow.close();
    	attachmentAddWindow.close();
     });
