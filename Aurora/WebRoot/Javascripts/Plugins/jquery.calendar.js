...
(function($) {
    ...
    $.fn.bcalendar = function(option) {
       ...
       // var __SCOLLEVENTTEMP = "<DIV style=\"WIDTH:${width};top:${top};left:${left};\" title=\"${title}\" class=\"chip chip${i} ${drag}\"><div class=\"dhdV\" style=\"display:none\">${data}</div><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}\" class=ct>&nbsp;</DIV><DL style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BACKGROUND-COLOR:${bgcolor1}; BORDER-TOP-COLOR: ${bdcolor}; HEIGHT: ${height}px; BORDER-RIGHT-COLOR:${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\"><DT style=\"BACKGROUND-COLOR:${bgcolor2}\">${starttime} – ${endtime} ${icon}</DT><DD><SPAN>${content}</SPAN></DD><DIV class='resizer' style='display:${redisplay}'><DIV class=rszr_icon>&nbsp;</DIV></DIV></DL><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BACKGROUND-COLOR:${bgcolor1}; BORDER-TOP-COLOR: ${bdcolor}; BORDER-RIGHT-COLOR: ${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\" class=cb1>&nbsp;</DIV><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BORDER-TOP-COLOR:${bdcolor}; BORDER-RIGHT-COLOR:${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\" class=cb2>&nbsp;</DIV></DIV>";
        var __SCOLLEVENTTEMP = "<DIV style=\"WIDTH:${width};top:${top};left:${left};\" title=\"${title}\" class=\"chip chip${i} ${drag}\"><div class=\"dhdV\" style=\"display:none\">${data}</div><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}\" class=ct>&nbsp;</DIV><DL style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BACKGROUND-COLOR:${bgcolor1}; BORDER-TOP-COLOR: ${bdcolor}; HEIGHT: ${height}px; BORDER-RIGHT-COLOR:${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\"><DT style=\"BACKGROUND-COLOR:${bgcolor2}\">${starttime} ${icon}</DT><DD><SPAN>${content}</SPAN></DD><DIV class='resizer' style='display:${redisplay}'><DIV class=rszr_icon>&nbsp;</DIV></DIV></DL><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BACKGROUND-COLOR:${bgcolor1}; BORDER-TOP-COLOR: ${bdcolor}; BORDER-RIGHT-COLOR: ${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\" class=cb1>&nbsp;</DIV><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BORDER-TOP-COLOR:${bdcolor}; BORDER-RIGHT-COLOR:${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\" class=cb2>&nbsp;</DIV></DIV>";
        ...
        //初始化高度
        gridcontainer.css("overflow-y", "visible").height(option.height - 10);
        
        //viewCenter();
        gridcontainer.load(viewCenter);
        $(window).resize(viewCenter);
        ...
        var viewCenter = function(){
	        ...
	        //初始化高度
	        gridcontainer.css("overflow-y", "visible").height(option.height - 10);
        	 var w = $("#add-calendar").attr("offsetWidth");
	         var h = $("#add-calendar").attr("offsetHeight");
	         
	         var top = $.getViewportHeight();
	         var left = $.getViewportWidth();
	         ...
	         $("#request-dialog, #reserve-dialog, #feedback-dialog").css({
	         	top:(top-h)/2,
	         	left:(left-w)/2
	         });
	         
	         $('#upload_flie_dialog').css({
	         	top: (top-$('#upload_flie_dialog').attr("offsetHeight"))/2,
	         	left: (left -$('#upload_flie_dialog').attr("offsetWidth"))/2
	         });
	         
	         $("#add-calendar").hide();
	         $("#show-calendar").hide();
	         $("#request-dialog").hide();
	         $("#setup-dialog, #reserve-dialog, #feedback-dialog").hide();
	         $('#upload_flie_dialog').hide();
        ...
        function BuildDayEvent(theme, e, index) {
            ...
            p.top = sP + "px";
            //p.left = (e.left * 100) + "%"; //跨整行
            p.left = "0%";
            //p.width = (e.aQ * 100) + "%"; //跨整行
            p.width = "100%";
            p.height = (eP - sP - 4);
            p.i = index;
        ...
        function CalDateShow(startday, endday, isshowtime, isshowweek) {
            if (!endday) {
              ...
              //  return startday.Format(getymformat(startday, null, isshowtime, isshowweek)) + "-" + endday.Format(getymformat(endday, startday, isshowtime, isshowweek));
                return startday.Format(getymformat(startday, null, isshowtime, isshowweek));
            }
        }
        ...
        function parseED(data) {
            ...
            if (data.length > 6) {
               ...
               //   e.push(data[0], data[1], data[2], data[3], parseInt(data[4]), parseInt(data[5]), parseInt(data[6]), data[7] != undefined ? parseInt(data[7]) : -1, data[8] != undefined ? parseInt(data[8]) : 0, data[9], data[10],data[11],data[10]);
                return e;
            }
            return null;
        ...
        function getbuddlepos(x, y) {
            ...
            return { left: (maxLeft-w)/2, top: (maxTop-h)/2, hide: true };
        }
            ...
            if (data != null) {
                if (option.quickDeleteUrl != "" && data[8] == 1 && option.readonly != true) {
                    ...
                    for(var i=0;i<option.eventItems.length;i++){
                    	if(option.eventItems[i][0] == data[0]){
                    		ee = option.eventItems[i];
                    		break;
                    	}
                    }
        ...
        function quickadd(start, end, isallday, pos) {
            ...
            $("#add-bottom-button-next-two").click(function(){                                      
                    ...
                    option.onBeforeRequestData && option.onBeforeRequestData(2);
                  	$.post(option.quickAddUrl, str, function(data) {
                        if (data) {
                     ...
                     var sd = strtodate($('#add-title-form [name="startTime"]').val());
                     var ed = strtodate($('#add-title-form [name="endTime"]').val());
