...
public class CriteriaFactory {
	...
	public void addLIKEArray(Criteria criteria){
		...
		while(ite.hasNext()){
			...
			log.debug("addLIKEArray,key="+key+"  valye="+value);
			if(value != null){
	...
	private void setPage(Criteria criteria){
		 ...
		 if(pbean != null){
			 criteria.setFirstResult(pbean.getStartSize());
			 criteria.setMaxResults(pbean.getPageSize());
		 }
	}
		...
		for(int i=0;i<size;i++){
			...
			if(type == CirteriaQuery.IS_INT){
				...
				if(val != -1){
					...
					if(a != null){
						System.out.println("alias "+a[0]+","+a[1]);
						bean.alias.put(a[0], a[1]);
					}
