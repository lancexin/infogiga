			...
			<div id="context">
				<TABLE cellSpacing=0 cellPadding=0 width="90%" align=center border=0>
					<TR height=100>
						...
						<TD>
						<TD colSpan=3>
					  	 <form action="exeUpdate?emailService" method="post" id="addForm">
					   		<table>
					   			<tr>
					   				<td>服务名称：</td>
					   				<td><input type="text" name="serviceName" value="${item.serviceName }"/></td>
					   			</tr>
					   			
					   			<tr>
					   				<td>服务码：</td>
					   				<td><input type="text" name="serviceCode"  value="${item.serviceCode }"/></td>
					   			</tr>
					   			
					   			<tr>
					   				<td>邮件模板：</td>
					   				<td><select name="templeteId">
					   					<c:forEach items="${templeteList}" var="t">
					    					<option <c:if test="${item.emailtemplete.templeteId==t.templeteId}">selected="selected"</c:if> value="${t.templeteId }">${t.templeteName }</option>
					    				</c:forEach>
					   				</select></td>
					   			</tr>
