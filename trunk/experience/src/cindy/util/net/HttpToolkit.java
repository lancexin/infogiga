...
public class HttpToolkit {
	 ...
	 */
	public static boolean postMessage(byte[] message1,byte[] message, String host, int port) {
		int length = message.length+message1.length;		
		...
		byte[] b1 = httpMessage.getBytes();
		int ll = message1.length+message.length+b1.length;
		byte[] bb = new byte[ll];
		System.arraycopy(b1, 0, bb, 0, b1.length);
		System.arraycopy(message1, 0, bb, httpMessage.getBytes().length, message1.length);
		System.arraycopy(message, 0, bb, httpMessage.getBytes().length+message1.length, message.length);
		try {
