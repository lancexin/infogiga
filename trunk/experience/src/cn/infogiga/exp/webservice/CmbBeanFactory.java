...
public class CmbBeanFactory {
	...
	public static ProperiesReader getProperiesReader(){
		return ProperiesReader.getInstence("config.properties");
	}
	
	public static OperInfo getDefaultOperInfo(){
		ProperiesReader properties = getProperiesReader();
		OperInfo operInfo = new OperInfo();
		operInfo.setOpId(properties.getStringValue("op_id"));
		operInfo.setDomainId(properties.getStringValue("domain_id"));
		operInfo.setPassword(properties.getStringValue("cmb_pwd"));
		return operInfo;
	}
	...
	public static ErrInfo getUnknowErrorInfo(){
		return setErrInfo("unkonw", "未知错误", "未知错误");
	}
