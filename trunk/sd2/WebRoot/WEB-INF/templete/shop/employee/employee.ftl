var newComponent = new Ext.ux.CommonTabPanel({
	id:"${code}",
	title:"${name}",
	storeUrl:"manager?shopemployee&type=json",
   	addUrl:"manager?employeeadd&type=json",
   	updateUrl:"manager?employeeupdate&type=json",
    ...
    gridAutoExpandColumn:"employeeId",
   	comboStore:[new Ext.data.JsonStore({ 
   		storeId:"lowPowerStore",
		autoLoad : true,
		idProperty: 'powerId',  	
		url :'power?comboLowPower&type=json', 
	...
	getAddForm:function(){
    	...
    	var addPowerCombo = new Ext.form.ComboBox({ 
			...
			y: 90,
			width:127,
			store : Ext.StoreMgr.get("lowPowerStore"), 
